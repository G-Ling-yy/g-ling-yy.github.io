...
import amapPlayer from './AmapPlayerDialog.vue'

//import axios from 'axios';


/*import g2json from '../../../static/G2.json';
import roadjson from '../../../static/t_road_line.json';*/
...
export default {
    ...
    data(){
        ...
        return {
            ...
            cameraGifUrl                     : {
              ...
              '2':require('@/assets/map/icon/actCam.gif'),
              '3':require('@/assets/map/icon/actCam.gif'),
              '4':require('@/assets/map/icon/actCam.gif'),
              '5':require('@/assets/map/icon/actCam.gif'),
              '6':require('@/assets/map/icon/actCam.gif'),
              '7':require('@/assets/map/icon/actCam.gif')
            },
            //摄像机正常图标
            cameraImgUrl                     : {
                '1':require('@/assets/map/icon/camera_icon2.png'),
                '2':require('@/assets/map/icon/camera_icon2.png'),
                '3':require('@/assets/map/icon/camera_icon2.png'),
                '4':require('@/assets/map/icon/camera_icon2.png'),
                '5':require('@/assets/map/icon/camera_icon2.png'),
                '6':require('@/assets/map/icon/camera_icon2.png'),
                '7':require('@/assets/map/icon/camera_icon2.png')

            },
            //摄像机离线图标
            cameraOfflineImgUrl              : {
              '1':require('@/assets/map/icon/camera_icon3.png'),
              '2':require('@/assets/map/icon/camera_icon3.png'),
              '3':require('@/assets/map/icon/camera_icon3.png'),
              '5':require('@/assets/map/icon/camera_icon3.png'),
              '6':require('@/assets/map/icon/camera_icon3.png'),
              '7':require('@/assets/map/icon/camera_icon3.png')
            },
            //摄像机故障图标
            cameraErrorImgUrl                : {
              '1':require('@/assets/map/icon/camera_icon0.png'),
              '2':require('@/assets/map/icon/camera_icon0.png'),
              '3':require('@/assets/map/icon/camera_icon0.png'),
              '5':require('@/assets/map/icon/camera_icon0.png'),
              '6':require('@/assets/map/icon/camera_icon0.png'),
              '7':require('@/assets/map/icon/camera_icon0.png')
            },
            cameraIconWidth                  : 18,//30,
            cameraIconHeight                 : 18, //35
        ...
        this.$nextTick(()=>{
            ...
            that.mapInstance.on('complete', function () {
                ...
                that.mapInstance.on('zoomend', function(e){
                    ...
                    that.currentMapZoom = zom;
                    console.info('zoomend  ',e,zom,that.districtExplorer.getAllFeaturePolygons());
                } );
                //地图移动
    ...
    methods:{
        ...
        showLineLayer(){
            ...
            let wms  = new AMap.TileLayer.WMS({
                ...
                params:{
                    ...
                    "BBOX": "12653649.606408,2625087.7436574,12697806.169576,2656394.0569978",
                    "WIDTH": 512,
                    "HEIGHT": 363
                }
            });
        ...
        initMapContent(){
            ...
            that.mapInstance.AmbientLight = new AMap.Lights.AmbientLight([1, 1, 1], 0.3);
            that.mapInstance.DirectionLight = new AMap.Lights.DirectionLight([0, 0, 1], [1, 1, 1], 0.5);
            ...
            AMapUI.load(['ui/geo/DistrictExplorer', 'lib/$'], function (DistrictExplorer, $) {
                ...
                distExp.on('featureClick', function (e, feature) {
                    ...
                    if(event.target.className.indexOf('overlay-title') >= 0 ||
                ...
                distExp.on('outsideClick', function (e) {
                    ...
                    //console.info('featureClick ', e);
                    distExp.locatePosition(e.originalEvent.lnglat, function (error, routeFeatures) {

                        //console.info('featureClick ', routeFeatures);
                        let addcode = that.baseTopAdcodes,adname = that.baseTopAdcodesName,level = '';
                        ...
                        /!*if (routeFeatures && routeFeatures.length > 1) {
                            ...
                            if(that.baseTopAdcodes !== 100000){
                                let baseNum = parseInt((''+that.baseTopAdcodes).substr(0,2)),
          ...
          if(list.length){
            ...
            for (let i = 0, len = num; i < len; i++) {
              let marker = new AMap.Marker({
                ...
                extData:{
                  onlineStatus:list[i].onlineStatus === null ? 0 : list[i].onlineStatus,
              ...
              let sta = ['<span style="color:#b1bfdc">(离线)</span>','','<span style="color:#fe3708">(故障)</span>'][list[i].onlineStatus];
              marker.content = `K${list[i].cameraKmpile?list[i].cameraKmpile:''}${list[i].cameraHmpile?'+'+list[i].cameraHmpile:''}${typeof (sta) === 'undefined' ?  ' (未知)'+list[i].onlineStatus : sta}`;
              ...
              marker.on('click', function(a){
                ...
                */
                if(cameraInfo.onlineStatus === '1') that.cameraInfoWindowShow({data:cameraInfo}, a.pixel || null);
              });
        ...
        renderAreaPolygons(areaNode) {
            ...
            fillOpacity = this.mapLayerType === 'google' ? 0 : 1;// 0.05*/


          let fillColor = this.mapLayerType === "google" ? 0 : "#1164ed",
            strokeColor = "#08142b", // '#0c3574',
            fillOpacity = this.mapLayerType === "google" ? 0 : 0.01;// 1
        ...
        districtHover(adcode){
            this.districtExplorer.setHoverFeature(this.currentAreaNode.getSubFeatureByAdcode(adcode));
        },
        ...
        cameraInfoWindowShow(record,pixel){
            ...
            if(record && record.data && record.data.cameraName){
                ...
                if(pixel && pixel.x && pixel.y){
                  ...
                  that.$refs.amapPlayer.setWindowSite(15 , 370);
                }
                setTimeout(()=>{
        ...
        addEventMarker(obj, flag =false){
            ...
            let ctype = data.classify === '0' || data.classify === '9' ? '1' : data.classify;
            ...
            let sta = ['<span style="color:#b1bfdc">(离线)</span>','','<span style="color:#fe3708">(故障)</span>'][data.onlineStatus];
            eventCameraMarker.content = `K${data.cameraKmpile?data.cameraKmpile:''}${data.cameraHmpile?'+'+data.cameraHmpile:''}${sta || ''}`;
            eventCameraMarker.on('mouseover', (e)=>{
                that.infoWindow.setContent(e.target.content);
                that.infoWindow.open(that.mapInstance, e.target.getPosition());
                that.tipMarker.hide();
            });
            eventCameraMarker.on('mouseout', (e)=>{
              that.infoWindow.close();
              that.tipMarker.show();
            });
        ...
        loadDefaultAmapEvent(){
            ...
            this.$root.$on("removeEventMarker",(nv)=>{
                if(this.backMark) this.removeMarker(this.backMark);
                if(this.chosedMark) this.chosedMark.show();
            });

            //设置marker点位置
            this.$root.$on("editEventMarkerLonlat",(nv)=>{
                if(this.backMark) this.backMark.setPosition(nv);
            });
        },
