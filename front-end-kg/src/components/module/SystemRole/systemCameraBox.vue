  ...
  <div class="content-wrapper camera-manage-wrapper" ref="viewbox">
    ...
    <div class="camera-manage-con" style="{height: calc(100% - 14px - 20px)}">
      <div class="cameraheader" v-if="!isCloudGateway">
        ...
        <el-button type="primary" plain class="querytt" @click="exportCameraList()">数据导出</el-button>
      ...
      <div>
        ...
        <div slot="header" class="clearfix card-province">
          ...
          <el-tag type="success" v-if="item.reportSource===1" style="float: right; padding: 3px 10px"> 网关 </el-tag>
        ...
        <div v-if="item.reportSource===1" class="ButtonGroup">
          <router-link :to="'/systemCameraBoxDetail?deviceCode='+item.deviceCode + '&title=待审核 (' + item.totalConfirmCameraNum + ')&id=' + item.transcodingId">
            <el-button class="Pending">待审核(<span>{{item.totalConfirmCameraNum}}</span>)</el-button>
          </router-link>
          <router-link :to="'/systemCameraPassDetail?deviceCode='+item.deviceCode + '&title=未通过 (' + item.rejectCameraNum + ')&id=' + item.transcodingId">
          <el-button class="Pass">未通过(<span>{{item.rejectCameraNum}}</span>)</el-button>
          </router-link>
        </div>
...
.camerabox-cards li:first-child,.platform-cards li:first-child{
  margin-top: 20px;
}
.reportSourceTwo li{
  list-style: none;
