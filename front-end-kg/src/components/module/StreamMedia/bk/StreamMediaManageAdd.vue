    ...
    submit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          $http.post("/sm/add", this.addData).then(res => {
            var res = res.data;
            if (res.code == 200) {
              this.$message({
                message: "保存成功",
              ...
              (findComponentUpward(this, "streamMedia").postData = {
                currPage: 1,
                pageSize: 3,
                smName: "",
                smType: "",
                ...
                findComponentUpward(this, "streamMedia").getData();
            } else {
              this.$message.error(res.message);
            }
          });
...
.gd-custom-dialog /deep/ .el-dialog__body {
  padding: 20px;
}
