...
<div 
    ...
    <el-form  
        ...
        :class="{'word-required-type':requiredWordType}"
        :inline="inline"
        ...
        >
        <div 
            v-for="(rowt, index) in rows" 
            :key="index"
            class="form-group-wrap">
            <div v-if="rowt.groupName && groupNameVisible" class="group-name" :class="{[groupNameClass]:true}" :data-anchor="rowt.groupName"
                 ref="groupName">{{rowt.groupName}}
            </div>
            <!-- <el-row :gutter="16" v-for="(row, index) in rowt.rows" :key="(rowt.groupName || '') + index"> -->
            <div v-for="(row, index) in rowt.rows" :key="(rowt.groupName || '') + index">
                <el-col :span="item.inerspan" v-for="item in row.cols" :key="item.name" :class="{[item.className]:!!item.className, 'changed':showChangeTip && changedItem[item.name]}">
                    <template v-if="!item.hidden">
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="isViewMode" :prop="item.name">
                            ...
                            <template v-else>
                                <component :ref="item.name" :is="item.component" :config="item"
                                           v-model="getLastObj(item.name)[getLastName(item.name)]" :is-view-mode="true"
                                           :parent="parent"></component>
                            </template>
                        </el-form-item>
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'text'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                <span v-html="item.label" class="form-plain-type-label"></span>
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'input'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                            <span slot="label">
                                <span v-html="item.label" class="form-plain-type-label"></span>
                                <el-tooltip :content="item.tip" v-if="item.tip">
                                    <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <el-input 
                                ...
                                :clearable="item.password?false:item.clearable"
                                :style="{width: getItemWidth(item.width) || innerItemWidth}"
                                :type="(item.password && item.passwordVisible)?'password':'text'"
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'time' || item.type == 'timerange'"
                                  :prop="item.name"  :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'number'" :prop="item.name"  :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <el-input-number :ref="item.name" :max="item.max === undefined?Infinity:item.max" :min="item.min === undefined? -Infinity:item.min"
                                         v-model="getLastObj(item.name)[getLastName(item.name)]"
                                         :style="{width: getItemWidth(item.width) || innerItemWidth}"
                                         :disabled="getDisabled(item)"
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'radio' || item.type === 'btnRadio'" :prop="item.name"  :rules="isViewMode?{}:item.validator">
                            <span slot="label">
                                <span v-html="item.label" class="form-plain-type-label"></span>
                                <el-tooltip :content="item.tip" v-if="item.tip">
                                    <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <el-radio-group 
                                :ref="item.name" 
                                v-model="getLastObj(item.name)[getLastName(item.name)]"
                                :disabled="item.disabled || isViewMode"
                                :size="item.size"
                                ...
                                <template v-if="item.type === 'btnRadio'">
                                    <el-radio-button
                                        ...
                                        :label="radio.id"
                                        :name="radio.name"
                                        :disabled="getDisabled(item) || radio.disabled"
                                        >
                                        {{ radio.label }}
                                    </el-radio-button>
                                </template>
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'checkbox'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <el-checkbox-group 
                                ...
                                <el-checkbox 
                                 ...
                                 <template v-if="item.style === 'button'">
                                    <el-checkbox-button
                                        v-for="op in item.optionsList"
                                        :key="op.id"
                                        :label="op.id"
                                        :true-label="op.trueLabel"
                                        :false-label="op.falseLabel"
                                        :disabled="getDisabled(item) || it.disabled"
                                        :name="op.name"
                                        :checked="op.checked"
                                        >
                                        {{ op.label }}
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'switch'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                        ...
                        <el-form-item
                            v-show="item.isShowOrganization"
                            ...
                            <el-cascader
                                ...
                                :disabled="getDisabled(item) || isViewMode"
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'select' && !item.spread" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <el-select :ref="item.name" :disabled="getDisabled(item)"
                                    v-model="getLastObj(item.name)[getLastName(item.name)]" :label-in-value="true"
                                    :transfer="true"
                                    :clearable="item.clearable"
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'textarea'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <div style="position:relative" :style="{width: getItemWidth(item.width) || innerItemWidth}">
                                <el-input 
                                    :ref="item.name" type="textarea" :autosize="item.autosize"
                                         v-model="getLastObj(item.name)[getLastName(item.name)]"
                                         :rows="item.rows"
                                         :clearable="item.clearable"
                                         :placeholder="item.placeholder"
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'custom'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'remote'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <el-select :ref="item.name"
                                    ...
                                    filterable
                                    remote
                                    :label="model[item.name+'_display']"
                                    :remote-method="remoteQuery(item)"
                                    :label-in-value="true"
                                    :loading="remoteLoading[item.name]">
                        ...
                        <el-form-item :label-width="item.labelWidth" :label="item.label" v-else-if="item.type == 'tree'" :prop="item.name" :rules="isViewMode?{}:item.validator">
                                <span slot="label">
                                    <span v-html="item.label" class="form-plain-type-label"></span>
                                    <el-tooltip :content="item.tip" v-if="item.tip">
                                        <i class="el-icon-info c-font-sub-color" size="14"></i>
                            ...
                            <tree-select
                                    ...
                                    :placeholder="item.placeholder"
                                    :clearable="item.clearable"
                                    :valueConsistsOf="item.valueConsistsOf"
