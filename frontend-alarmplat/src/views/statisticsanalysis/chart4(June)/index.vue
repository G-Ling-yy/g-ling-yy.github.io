<template>
  <div class="full-wrap">
    <!-- 表单 -->
    <SelfForm :curChartType="curChartType" @handleSearch="getChartData" />

    <main>
      <div class="charts-wrap">
        <!-- 柱状图 -->
        <BarChart v-if="curChartType === 'bar'" :data="chartData" :loading="loading" />

        <!-- 饼图 -->
        <!-- <PieChart v-if="curChartType === 'pie'" :loading="evtsLoading" /> -->

        <!-- 折线图 -->
        <LineChart v-if="curChartType === 'line'" :data="chartData" :loading="loading" />
      </div>
      ...
      <div class="charts-switches">
        <div
          ...
          :class="['switch btn', chartType, chartType === curChartType && 'active']"
          :key="chartType"
...
const getChartData = chartType => {
  ...
  // 厂商参数
  const corps = []
  for (const key in formData.value.corps[formData.value.isPoc]) {
    formData.value.corps[formData.value.isPoc][key] && corps.push(key)
  }
