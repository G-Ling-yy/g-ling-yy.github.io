...
const getChartData = () => {
    !myChart &&
      (myChart = echarts.init(statisticsChartRef.value))

    myChart?.showLoading()
    apis.events
      .getCumulativeStatisticsChart({
        ...formData.value,
  ...
  handleChartData = data => {
    data.sort((a, b) =>
      a.dataSourceDate > b.dataSourceDate ? 1 : -1
    )
    const xAxisData = [],
      legendData = [],
      legendSelected = {},
      ...
      PreAryObj = {
        ...
        累计检出率: {
          propName: 'checkRateCumulative',
          data: []
        },

        '0-100米检出率': {
          propName: 'checkRate100',
          data: []
        },

        '100-200米检出率': {
          propName: 'checkRate200',
          data: []
        },

        '200-300米检出率': {
          propName: 'checkRate300',
          data: []
        },

        '300-400米检出率': {
          propName: 'checkRate400',
          data: []
        },

        '400-500米检出率': {
          propName: 'checkRate500',
          data: []
        },

        '500米以上检出率': {
          propName: 'checkRatePass500',
          data: []
        },

        // 累计800米检出率: {
        //   propName: 'checkRate800',
        //   data: []
        // },
        ...
        累计正确率: {
          propName: 'correctRateCumulative',
          data: []
        },
        ...
        累计报错率: {
          propName: 'errorRateCumulative',
          data: []
        },

        累计误警率: {
          propName: 'falseAlarmRateCumulative',
          data: []
        },
    ...
    data.forEach(e => {
      ...
      keys.forEach(key => {
        PreAryObj[key].data.push(
          e.id ? trans2Num(e[PreAryObj[key].propName]) : 0
        )
      })
    })
  ...
  renderChart = ({
    ...
    const option = {
      ...
      yAxis: {
        ...
        axisLabel: {
          formatter: `{value}${
            chartTypeObj[chartType.value].yAxisUnit
          }`
        }
      },
